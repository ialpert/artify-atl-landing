  ...
  return (
    <>
      {blogData.slice(29, 32).map((blog, index) => (
        <div key={index} className={`col-lg-4 col-sm-6 d-flex`} data-aos="fade-up" data-aos-delay={`${index * 100}`}>
          <article className="blog-meta-one tran3s mt-30">
            <figure className="post-img m0">
              <Link href={`/blog/${blog.id}`} className="w-100 d-block">
                <img src={blog.imageSrc} alt="icon" className="lazy-img w-100 tran4s" />
              </Link>
            </figure>
