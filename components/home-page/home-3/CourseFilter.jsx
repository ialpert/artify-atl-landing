...
const CourseFilter = () => {
  ...
  const filteredItems = filter === '*' ? courseData : courseData.filter((item) => item.tags.includes(filter));
  ...
  return (
    <>
      <ul className="nav nav-tabs border-0 mb-130 lg-mb-80 md-mb-30" role="tablist" data-aos="fade-up">
        <ul className="style-none text-center isotop-menu-wrapper d-flex flex-wrap">
          ...
          <li className="nav-item">
            <button className={filter === 'it' ? 'active nav-link' : 'nav-link'} onClick={() => setFilter('it')}>
              IT
            </button>
          </li>
          <li className="nav-item">
          ...
          <li className="nav-item">
            <button className={filter === 'art' ? 'active nav-link' : 'nav-link'} onClick={() => setFilter('art')}>
              Art
            </button>
          </li>
          <li className="nav-item">
            <button
              className={filter === 'marketing' ? 'active nav-link' : 'nav-link'}
      ...
      <div className="slider-wrapper">
        <div className="tab-content position-relative zn2 course_slider_one" data-aos="fade-up">
          <Slider {...settings} arrows={false}>
            {filteredItems.map((course) => (
              <div className="item" key={course.id}>
                <div className="card-style-twenty tran3s">
                  <div className="img-meta">
                    <Image width={315} height={222} src={course.imgSrc} alt="media" className="w-100" />
                  </div>
                  {/* /.img-meta */}
                  <div className="course-data">
                    <div className="d-flex align-items-center justify-content-between">
                      <div className="course-by fs-17 tx-dark opacity-75">{course.instructor}</div>
                      <div className="course-duration fs-17 tx-dark">{course.duration}</div>
                    </div>
                    ...
                    <div className="course-review d-flex align-items-center">
                      ...
                      <span className="fs-15 tx-dark ms-1">({course.ratingsCount})</span>
                    </div>
                    <div className="d-flex align-items-center justify-content-between mt-30 mb-10">
                      <strong className="price fw-500 fs-20 tx-dark">
                        ...
                        <del className="opacity-50 fw-normal">${course.discountedPrice.toFixed(2)}</del>
                      </strong>
